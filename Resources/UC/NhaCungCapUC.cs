    ...
    public partial class NhaCungCapUC : UserControl
    {
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Vui lòng nhập đủ thông tin nhà cung cấp.");
                    return; // Không thêm nhà cung cấp nếu thiếu thông tin
                }

                // Kiểm tra xem nhà cung cấp đã tồn tại trong cơ sở dữ liệu chưa
                string soDienThoai = txtSoDienThoai.Text;
                var existingSupplier = db.NhaCungCaps.FirstOrDefault(s => s.SoDienThoai == soDienThoai);

                if (existingSupplier != null)
                {
                    MessageBox.Show("Nhà cung cấp đã tồn tại trong cơ sở dữ liệu. Vui lòng kiểm tra lại.");
                    return;
                }
