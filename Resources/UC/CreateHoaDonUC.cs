...
namespace EliteMart.UC
{
    public partial class CreateHoaDonUC : UserControl
    {
        ...
        private void btnTaoHoaDon_Click(object sender, EventArgs e)
        {
            try
            {
                // Kiểm tra xem các trường thông tin cần thiết đã được nhập đầy đủ chưa
                if (string.IsNullOrWhiteSpace(txtHoTen.Text) || string.IsNullOrWhiteSpace(txtDiaChi.Text) ||
                   string.IsNullOrWhiteSpace(cbxGioiTinh.Text) || string.IsNullOrWhiteSpace(txtSoDienThoai.Text) ||
                   chiTietHoaDons.Count == 0)
                {
                    MessageBox.Show("Vui lòng nhập đầy đủ thông tin và thêm ít nhất một sản phẩm vào hóa đơn.");
                    return; // Không tạo hóa đơn nếu thiếu thông tin
                }

                // Tạo mới đối tượng HoaDon và gán thông tin từ các trường nhập liệu
                ...
                // Kiểm tra xem có đủ hàng cho các sản phẩm trong hóa đơn không
                foreach (var item in chiTietHoaDons)
                {
                    HangHoa hangHoa = db.HangHoas.Find(item.MaHangHoa);
                    if (hangHoa.SoLuong < item.SoLuong)
                    {
                        MessageBox.Show("Không đủ hàng cho sản phẩm: " + hangHoa.TenHangHoa);
                        return;
                    }
                }

                // Trừ số lượng sản phẩm từ kho hàng trước khi tạo hóa đơn
                foreach (var item in chiTietHoaDons)
                {
                    HangHoa hangHoa = db.HangHoas.Find(item.MaHangHoa);
                ...
                // Lưu hóa đơn và cập nhật số lượng hàng trong kho vào cơ sở dữ liệu
